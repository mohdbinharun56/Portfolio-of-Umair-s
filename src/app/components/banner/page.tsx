export default function Banner({ informations }: any) {
    ...
    return (
        <>
            <div id="banner" className="lg:max-w-7xl md:flex mx-auto mt-20">
                <div className="w-2/3 ml-8 lg:ml-24">
                    ...
                    <p className="w-96 font-bold text-6xl lg:text-7xl">{title ? title.slice(0, 7) : ''}</p>
                    <p className="lg:w-80 font-bold text-6xl mx-auto lg:text-7xl ">{title ? title.slice(7) : ''}</p>
                    <p className="font-normal lg:text-xl my-6">{aboutMe? aboutMe.map((about: { description: any; })=>about.description):''}</p>
                    <button className="w-40">Hire me</button>
                </div>
                <div className="circle w-52 h-52 lg:w-96 lg:h-96 rounded-full relative top-20">
                    <div>
                        <img src={profileImage} alt="profilePhoto" className="w-[280px] h-[280px] lg:w-[450px] lg:h-[450px] absolute left-1 -top-[70px] rounded-full mx-auto" />
                        <div className="w-40 h-8 lg:w-72 lg:h-[70px] border-1 bg-[#FD6F0099] absolute ml-8 -mt-5"></div>
                    </div>
                </div>
            </div>
